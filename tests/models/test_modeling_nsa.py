...
pytestmark = pytest.mark.skipif(find_spec_cached(
    "flash_attn") is None, reason="NSA sliding-window attention requires flash-attn (`pip install flash-attn --no-build-isolation`).")
...
def test_generation(
    ...
    run_test_generation(L, B, T, H, D, NSAConfig, dtype)
